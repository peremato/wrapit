...
std::ostream&
CodeTree::generate_cxx(std::ostream& o, std::string& odir){
  ...
  auto gen_comment_header = [&](std::ostream& o, const TypeRcd& t){
  ...
  indent(o << "\n", 1) << "/* call functions to add all methods for each wrapped type */\n";
  ...
  for(const auto& t: types_){
    ...
    auto subfilename = buf.str();
    std::ofstream o_(subfilename, std::ofstream::out);
    o_ << "// this file was auto-generated by wrapit " << version << "\n";
    o_ << "#include <type_traits>\n"
          "#include \"jlcxx/jlcxx.hpp\"\n"
          "#include \"jlcxx/functions.hpp\"\n"
          "#include \"jlcxx/stl.hpp\"\n\n";
    o_ << "#include " << filename_ << "\n\n";
    o_ << "\n#ifdef VERBOSE_IMPORT\n";
    indent(o_ << "#",1) << "define DEBUG_MSG(a) std::cerr << a << \"\\n\"\n";
    o_ << "#else\n";
    indent(o_ << "#",1) << "define DEBUG_MSG(a)\n";
    o_ << "#endif\n";

    o_ <<"#define __HERE__  __FILE__ \":\" QUOTE2(__LINE__)\n"
         "#define QUOTE(arg) #arg\n"
         "#define QUOTE2(arg) QUOTE(arg)\n";
    o_ << "void add_methods_for_" << type_fname << "(jlcxx::TypeWrapper<" << t.type_name << ">& t" << t.id << ") {" << std::endl;

    if(/*t.template_parameter_combinations.size() == 0
         &&*/ clang_getCursorKind(t.cursor)!= CXCursor_ClassTemplate){
      for(const auto& m: t.methods){
        gen_comment_header(o_, t);
        generate_method_cxx(o_, m);
        if(test_build_) test_build(o_);
      }
      if(accessor_generation_enabled()){
        for(const auto& f: t.fields){
          ...
          if(accessor_gen != accessor_mode_t::none){
            gen_comment_header(o_, t);
            generate_accessor_cxx(o_, &t, f, accessor_gen == accessor_mode_t::getter, 1);
          }
        }
      }
